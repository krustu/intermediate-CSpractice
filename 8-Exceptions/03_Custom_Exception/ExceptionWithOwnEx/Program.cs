...
class Program
{
    static void Main()
    {
        ...
        try
        {
            var withAmount = User1.Withdraw(4);
...
public class ExceptionInBalance : Exception
{
    public decimal RequestedAmount { get; }
    public decimal AvailableBalance { get; }
    public string Names { get; }
    public ExceptionInBalance(decimal RequestedAmount, decimal AvailableBalance, string Names)
        : base($"Dear {Names} you have - Insufficient funds:{RequestedAmount} requested, available {AvailableBalance}")
    {
...
class BankAccount
{
    ...
    public decimal Withdraw(decimal amount)
    {
        try
        {
            ...
            if (amount > _amount)
                throw new ExceptionInBalance(amount, _amount, Name);
        ...
        catch (ExceptionInBalance ex)
        {
            Console.WriteLine(ex.Message);
            decimal shortage = ex.RequestedAmount - ex.AvailableBalance;
            Console.WriteLine($"That is exactly how much is missing.: {shortage}");
            throw;
        }
